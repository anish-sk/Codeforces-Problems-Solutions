...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll q; cin>>q; vll w(q+1,0), depth(q+1, 0);
        vvl adj(q+1, vll());
        vvl par(20, vll(q+5, 0));
        vvl mx(20, vll(q+5, 0));
        vvl nxt(20, vll(q+5, 0));
        vvl sum(20, vll(q+5, 0));
        ll pi, qi;
        ll ty;
        ll cnt = 1;
        ll last = 0;
        ...
        forn(i,q){
            ...
            if(ty == 1){
                ...
                qi ^= last;
                adj[pi].pb(cnt);
                depth[cnt] = depth[pi]+1;
                par[0][cnt]=pi;
                w[cnt]=qi;
                forn(k, 19){
                    par[k+1][cnt] = par[k][par[k][cnt]];
                }
                mx[0][cnt]=w[pi];
                forn(k, 19){
                    mx[k+1][cnt] = max(mx[k][cnt],mx[k][par[k][cnt]]);
                }
                ll ver = cnt;
                for(ll k = 19; k>=0; k--){
                    if(!par[k][ver]) continue;
                    if(mx[k][ver] < qi){
                        ver = par[k][ver];
                    }
                }
                nxt[0][cnt] = par[0][ver];
                forn(k, 19){
                    nxt[k+1][cnt] = nxt[k][nxt[k][cnt]];
                }
                sum[0][cnt]=w[nxt[0][cnt]];
                forn(k, 19){
                    sum[k+1][cnt] = sum[k][cnt] + sum[k][nxt[k][cnt]];
                }
            }
            else{
                cin>>pi>>qi;
