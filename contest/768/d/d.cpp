...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll k,q; cin>>k>>q; vll p(q); cin>>p;
        vector<vector<ld>> f(10001, vector<ld>(k+1,0));
        vll fact(k+1, 1), powin(10001,1);
        f[1][1]=1;
       for(ll n = 2; n<=10000; n++){
        for(ll k1 = 1; k1<=min(n,k); k1++){
                f[n][k1] = static_cast<ld>(k1)*f[n-1][k1] + static_cast<ld>(k-k1+1)*f[n-1][k1-1];
                f[n][k1] /= static_cast<ld>(k);
            }
        }
        //for(int n=1; n<=100; n++){
        //    cout<<f[n][k]<<" ";
        ...
        //cout<<"\n";
        for(auto pi : p){
            ld thres = static_cast<ld>(pi)/static_cast<ld>(2000);
            ll n;
            for(n = 1; n<=1e4; n++){
                if(f[n][k] >= thres){
                    break;
