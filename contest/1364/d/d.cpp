...
const ll MAXN = 1e5+5;
vll parent, depth;
void dfs(vvl &adj,ll u, vll &cycle, bool &flag){
    for(auto v: adj[u]){
        ...
        if(depth[v]!=-1){
            ...
            if((ll)cycle.size()<=k){
                cout<<"2\n";
                for(int i=0;i<(int)cycle.size();i++) cycle[i]++;
                cout<<cycle.size()<<"\n";
                o1(cycle);
            }
            else{
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m; cin>>n>>m>>k; ll u,v; vvl adj(n, vll());
        forn(i,m){
            cin>>u>>v;u--;v--;
            adj[u].pb(v);
            adj[v].pb(u);
        ...
        depth[0]=0; vll cycle;
        dfs(adj,0,cycle,flag);
        if(!flag){
            vll one,two;
            for(int i=0;i<n;i++){
                if(depth[i]&1) one.pb(i+1);
                else two.pb(i+1);
            }
            cout<<"1\n";
            //vll om;
            if(one.size() > two.size()) {
                for(int i=0;i< ceil(k,2LL); i++) cout<<one[i]<<" ";
                cout<<"\n";
            }
            else{
                for(int i=0;i< ceil(k,2LL); i++) cout<<two[i]<<" ";
                cout<<"\n";
            }
        }
    }
