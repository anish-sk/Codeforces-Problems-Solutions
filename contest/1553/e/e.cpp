...
bool check(vll &a, ll k, ll m){
    ll n = a.size();
    vll b(a.size(), 0);
    forn(i, a.size()){
        b[i] = (i - k + n)%n;
    }
    vll parent(a.size(), 0), size(a.size(), 0);
    forn(i, a.size()){
        make_set(i, parent, size);
    }
    forn(i, a.size()){
        union_sets(a[i], b[i], parent, size);
    }
    set<ll> x;
    forn(i, a.size()){
        x.insert(find_set(i, parent));
    }
    return (n - (ll)x.size()) <= m;
}
int main()
{
