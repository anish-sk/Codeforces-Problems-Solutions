...
void calcdp(struct TrieNode *root){
    struct TrieNode *pCrawl = root;
    bool flag = 0;
    for(int i=0; i<ALPHABET_SIZE; i++){
        if(pCrawl->children[i]){
            flag = 1;
            calcdp(pCrawl->children[i]);
            pCrawl->win |= !((pCrawl->children[i])->win);
            pCrawl->lose |= !((pCrawl->children[i])->lose);
        }
    }
    if(!flag) {
        pCrawl->win = false;
...
int main()
{
    ...
    while(t--){
        ...
        forn(i,n) {cin>>a[i]; insert(root,a[i]);}
        calcdp(root);
