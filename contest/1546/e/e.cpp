...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n; cin>>n; vvl a(2LL*n, vll(n)); cin>>a;
        map<pll, set<ll>> com;
        forn(i,2LL*n){
            forn(j, n){
                com[{j, a[i][j]}].insert(i);
            }
        }
        set<ll> ones, total;
        forn(i,2LL*n){
        ...
        while(total.size()){
            if(ones.size()){
                ...
                set<ll> to_remove;
                to_remove.insert(ind);
                forn(j, a[ind].size()){
                    for(auto x: com[{j,a[ind][j]}]){
                        to_remove.insert(x);
                    }
                }
                set<pll> new_ly;
                for(auto ve: to_remove){
                    forn(i, a[ve].size()){
                        com[{i, a[ve][i]}].erase(ve);
                        if(com[{i, a[ve][i]}].size() == 1){
                            new_ly.insert({i, a[ve][i]});
                        }
                ...
                for(auto pa : new_ly){
                    if(com[pa].size() == 1) ones.insert(*com[pa].begin());
                }
                out.pb(ind+1);
            }
            else{
