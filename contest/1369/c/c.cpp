...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n,k; cin>>n>>k; vll a(n); cin>>a; vll w(k); cin>>w;
        int onec = 0;
        for(auto i: w) onec += (i==1);
        ...
        sort(all(wleft), greater<int>());
        for(int i=1;i<(int)wleft.size();i++){
            wleft[i] +=wleft[i-1];
        }
        if(unmarked.size() > 0)
        sum+=unmarked[0];
        //cout<<wleft<<flush;
        for(int i=0;i< (int)wleft.size() - 1; i++){
            sum+= unmarked[wleft[i]];
        }
        cout<<sum<<"\n";
