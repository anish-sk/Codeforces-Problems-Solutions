...
ll p =1e9+7;
bool solved(ll x, ll n, ll k, vll sum){
    vector<vector<bool>> dp(n+1, vector<bool>(k+1,0));
    dp[0][0] = 1;
    for(int r=1; r <=n; r++){
        for(int shelf=1; shelf<=k; shelf++ ){
            bool flag = 0;
            for(int left = 0; left<r;left++){
                flag |=(((sum[r] - sum[left]) & x) == x)*(dp[left][shelf-1]); 
            }
            dp[r][shelf] = flag;
        }
    }
...
int main()
{
    ...
    while(t--){
        ...
        for(ll i=60;i>=0;i--){
            if(solved(ans | 1LL<<i, n, k, sum)) ans|= 1LL<<i;
        }
        cout<<ans<<"\n";
    }
    return 0;
