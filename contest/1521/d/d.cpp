...
void dfs(vector<set<ll>> &adj,ll u, ll v, vll &deg, vvl &ans,vector<set<ll>> &adj2 ){
    ll child = 0;
    for(auto i : adj[u]){
        if(i==v) continue;
        dfs(adj, i, u, deg, ans, adj2);
        if(adj2[u].count(i))
        child++;
    }
    if(child>=2 and v!=-1){
...
void dfs2(vector<set<ll>> &adj,ll u, ll v, vll &deg, vll &leaves, vll &visited){
    visited[u]=1;
    for(auto i : adj[u]){
        if(i==v) continue;
        dfs2(adj, i, u, deg, leaves, visited);
    }
    if(deg[u] == 1) leaves.pb(u+1);
    if(deg[u] == 0){
        leaves.pb(u+1);
        leaves.pb(u+1);
    }
...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n; cin>>n; vector<set<ll>> adj(n, set<ll>()); ll u,v;
        vector<set<ll>> adj2(n, set<ll>());
        vll deg(n, 0);
        forn(i, n-1){
            cin>>u>>v; u--;v--;
            adj[u].insert(v);
        ...
        vvl ans;
        dfs(adj, 0, -1, deg, ans, adj2);
        vll leaf;
        vvl leaves;
        vll visited(n, 0);
        forn(i,n){
            if(visited[i]) continue;
            dfs2(adj2, i, -1, deg, leaf, visited);
            leaves.pb(leaf);
            leaf.clear();
        }
        for(int i=0; i<leaves.size()-1; i++){
            ans[i].pb(leaves[i][1]);
            ans[i].pb(leaves[i+1][0]);
        }
        cout<<ans.size()<<"\n";
        for(auto v: ans){
            for(auto i: v){
