...
ll get_next(ll x, ll z=0){
    ll y = max(x, z);
    if(z == 0){
        for(ll i=y+1; i<=2LL*y; i++){
            if(__gcd(i,x) == 1) return i;
        }
    }
    else{
        for(ll i=y+1; i<=2LL*y; i++){
            if(__gcd(i,x) == 1 and __gcd(i,z) == 1) return i;
        }
...
int main()
{
    ...
    while(t--){
        ...
        for(int i=0; i<n-1; i++){
            if(a[i] <= a[i+1]){
                a[i+1] = get_next(a[i]);
                cout<<i+1<<" "<<i+2<<" "<<a[i]<<" "<<a[i+1]<<"\n";
            }
            else{
                if(i>=1)
                a[i] = get_next(a[i+1], a[i-1]);
                else
                a[i] = get_next(a[i+1], a[i]);
                cout<<i+1<<" "<<i+2<<" "<<a[i]<<" "<<a[i+1]<<"\n";
                //swap(a[i+1], a[i]);
            }
        }
