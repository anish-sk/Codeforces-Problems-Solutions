...
int solve(vector<int> &c, int bit){
    if(bit == -1) return 0;
        ...
        vector<int> zer, one;
        for(auto str : c){
            if(((str>>bit)& 1) == 1) one.pb(str);
            else zer.pb(str);
        }
        if(one.size() == 0) { int a = solve(zer, bit-1);
        zer.clear(); one.clear(); return a;}
        else if(zer.size() == 0) {int b =  solve(one, bit -1); zer.clear(); one.clear(); return b;}
        else {
            int d = solve(zer, bit - 1);
            int e = solve(one, bit - 1);
            zer.clear(); one.clear();
            return (1<<bit) + min(d,e);  
        }
    }
}
int main()
{
    ...
    while(t--){
        ...
        //    c.pb(b);
        //}
        //zeros.resize(32, vi()); ones.resize(32, vi());
        //for(int i=31;i>=0;i--){
        //    for(int j=0;j<n;j++){
        //        if(c[j][i] == '1') ones[i].pb(j);
        //        else zeros[i].pb(j);
        //    }
        //}
        cout<<solve(a,30)<<"\n";
