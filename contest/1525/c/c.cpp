...
void calc(vector<pll> &y, vll &dir, vll &ans, ll m){
    stack<pll> st;
    for(auto pa: y){
        if(dir[pa.s] == 1){
            st.push(pa);
            ...
            pll c = st.top();
            st.pop();
            ans[c.s] = ans[pa.s] = (pa.f - c.f)/2;
        }
        else st.push({-pa.f, pa.s});
    }
    while(st.size()>=2){
        pll a = st.top();
        st.pop();
...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vll x(n); cin>>x; vector<char> s(n); cin>>s;
        vll dir; for(auto i: s) if(i == 'L') {dir.pb(-1);} else dir.pb(1);
        vector<pair<ll,ll>> yodd, yeven;
        forn(i,n) {
            if(x[i]&1){
                yodd.pb({x[i], i});
            }
