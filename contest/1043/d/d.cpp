    ...
    while(t--){
        ...
        forn(i,n){
            ...
            while(flag){
                set<int> nxt;
                forn(j,m){
                    visited[p[0][ptr[0]]-1]=1;
                    if(ptr[j]==n-1) {
                        flag = 0;
                        break;
                    }
                    else {
                        nxt.insert(p[j][ptr[j]+1]);
                    }
                }
                if((flag) && nxt.size()==1){ct++;
                    forn(j,m){
                        ptr[j]++;
                    }
                }
