...
struct LCA {
    ...
    bool test (int kan, int a, int b){
        int l = lca(a,b);
        if(child(l, kan) and child(kan, a)) return true;
        if(child(l, kan) and child(kan, b)) return true;
        return false;
    }
...
int main()
{
    ...
    while(t--){
        ...
        while(q--){
            ...
            int ca = lca.lca(c,a);
            int p1=0,p2=0,p3=0;
            vi temp; temp.pb(ab);temp.pb(bc); temp.pb(ca);
            int te=-1;
            for(auto i: temp){
                if(lca.test(i,a,b) and lca.test(i,b,c) and lca.test(i,c,a)){
                    te = i;
                    break;
                }
            }
            int ans = 0;
            if(te!=-1){
                ans = max(ans, lca.distance(te, a));
                ans = max(ans, lca.distance(te, b));
                ans = max(ans, lca.distance(te, c));
            }
            cout<<ans+1<<"\n";
            //if (bc == a) p1=0;
            //else if(bc == b or bc == c){
            //    p1 = min (lca.distance(a,b), lca.distance(a,c));
            //}
            //else{
            //    p1 = lca.distance(a,bc);
            //}
            //if (ca == b) p2=0;
            //else if(ca == a or ca == c){
            //    p2 = min (lca.distance(b,a), lca.distance(b,c));
            //}
            //else{
            //    p2 = lca.distance(b,ca);
            //}
            //if (ab == c) p3=0;
            //else if(ab == b or ab == a){
            //    p3 = min (lca.distance(c,b), lca.distance(c,a));
            //}
            //else{
            //    p3 = lca.distance(c,ab);
            //}
            //
