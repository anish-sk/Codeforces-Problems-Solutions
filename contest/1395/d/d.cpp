...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,d,m; cin>>n>>d>>m; vll a(n); cin>>a;
        vll big, small;
        for(auto &i: a){
            if(i>m) big.pb(i);
            else small.pb(i);
        ...
        ll ans = small.size()?small.back():0;
        ll l = small.size();
        forn(i, n - l + 1){
            small.push_back(ans);
        }
        //cout<<big<<small;
        for(ll x = 1LL; x<=big.size(); x++){
            ll e = big[x-1];
            ll rem = n - (x-1LL)*(d+1LL) - 1LL;
            //cout<<rem<<"\n";
            if(rem >=0 and rem<=small.size()){
                if(rem > 0)
                e+=small[rem-1];
                ans = max(ans, e);
            }
