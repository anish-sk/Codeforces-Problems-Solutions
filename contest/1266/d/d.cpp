...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vector<pair<ll,ll>> bal(n,{0LL,0LL});
        forn(i,m){
            ll u,v,d; cin>>u>>v>>d;u--;v--;
            bal[u]={bal[u].f+d,u};bal[v]={bal[v].f-d, v};
        }
        sort(all(bal));
        reverse(all(bal));
        int i=0,j=n-1;
        ...
        //cout<<bal
        while(i<j){
            if(bal[i].f == 0LL) {i++;continue;}
            if(bal[j].f == 0LL) {cout<<bal[i]<<" "<<i<<" "<<j<<" "<<bal[j+1]<<"\n";}
            if(bal[i].f + bal[j].f > 0){
                us.pb(bal[i].s); vs.pb(bal[j].s); cs.pb(abs(bal[j].f));
                bal[i].f= bal[i].f + bal[j].f;
                //bal[j].f = 0;
                j--;
            }
            else if(bal[i].f == -bal[j].f){
                us.pb(bal[i].s); vs.pb(bal[j].s); cs.pb(abs(bal[j].f));
                bal[i].f=0;bal[j].f=0;
                i++;j--;
            }
            else{
                us.pb(bal[i].s); vs.pb(bal[j].s); cs.pb(abs(bal[i].f));
                bal[j].f += bal[i].f;
                //bal[i]=0;
                i++;
            }
