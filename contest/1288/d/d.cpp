...
ll p =1e9+7;
void bitr(string &res, string a, string b){
    forn(i, a.size()){
...
bool solve(vvl &a, ll x, string &c, string &d){
...
int main()
{
    ...
    while(t--){
        ...
        ll l = 0, r = 1e9+2LL;
        ll mid = (l+r)/2LL;
        string c=string(m, '0'),d = string(m, '0');
        while(l < r){
            mid = (l+r)/2LL;
            if(solve(a, mid, c, d)){
                l = mid + 1LL;
        ...
        ll x=0;
        for(x = max(mid -10LL,1LL) ; x <= mid + 10LL; x++){
            if(!solve(a, x, c, d)) break;
        }
        solve(a,x-1,c,d);
        //cout<<mid<<" "<<x<<"\n";
        ll u=1,v=1;
