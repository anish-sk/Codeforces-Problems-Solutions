...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll q; cin>>q; vll c(q+1,0), a(q+1, 0);
        cin>>a[0]>>c[0];
        vvl adj(q+1, vll());
        vvl par(20, vll(q+1, 0));
        ll pi, ai, ci, v, wi;
        ll ty;
        forn(i,q){
            cin>>ty;
            if(ty == 1){
                ...
                while(qbou < wi){
                    ll ver = v;
                    for(ll k=19; k>=0; k--){
                        if(a[par[k][ver]] > 0) ver = par[k][ver];
                    }
                    //cout<<ver<<"\n";
                    if(a[ver] == 0) break;
                    ll bo = min(wi-qbou, a[ver]);
                    a[ver]-=bo;
                    qbou += bo;
                    cos += c[ver] * bo;
                    //cout<<"hi "<<qbou<<" "<<wi<<"\n";
