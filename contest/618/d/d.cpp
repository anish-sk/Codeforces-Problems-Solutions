...
void dfs(vvl &adj,ll u, ll v, ll &U, vll &count){
    //cout<<u<<flush;
    ll mark = 0;
    for(auto i : adj[u]){
        if(i==v) continue;
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,x,y; cin>>n>>x>>y; vvl adj(n); vll deg(n,0);
        forn(i,n-1){
            ll u,v; cin>>u>>v; u--;v--;
        ...
        if (x > y){
            ans = n*y-y;
            bool flag = 0;
            for(auto i: deg) if(i == n-1) flag = 1;
            if(flag) ans+= (x - y);
        }
        else{
            //forn(i,n) cout<<adj[i]<<flush;
            vll count(n,0); ll U=0;
            dfs(adj, 0, -1, U, count);
            //cout<<"hi"<<flush;
            //cout<<U<<"\n";
            ans = (n - 1 - U)*y + U*x;
