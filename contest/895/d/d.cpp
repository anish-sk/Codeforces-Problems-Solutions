...
vll fact(1e6+5, 1);
vll ifact(1e6+5, 1);
vll fa(1e6+5,1), ifa(1e6+5,1);
ll p =1e9+7;
ll per(vll &cnt, ll &sum){
...
int main()
{
    ...
    while(t--){
        ...
        forn(i,a.size()){
            ...
            ifact[i+1] = MINV(fact[i+1], p);
            fa[i+1] = i+1;
            ifa[i+1] = MINV(i+1, p);
            //cout<<fact[i]<<" ";
        }
        
        cout<<(f(b,a) - f(a,a) - 1 + p)%p<<"\n";    
    }
    return 0;
}
