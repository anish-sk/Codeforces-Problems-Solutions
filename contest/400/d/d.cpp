...
void dfsvisit(vvl &adj,vll &visited, ll v){
    visited[v]=1;
    for(auto i: adj[v]){
        if(!visited[i]){
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m,k; cin>>n>>m>>k; vll c(k); cin>>c;
        int ty = 0;
        vll type(n,0);
        forn(i,k){
            forn(j,c[i]){
        ...
        bool flag  = 0;
        vector<vector<ll>> adj(k, vll(k,1e8));
        vector<vector<ll>> graph(n, vll());
        forn(__, m){
            ll u,v,x; cin>>u>>v>>x; u--;v--;
            if(type[u]!=type[v]){
                adj[type[u]][type[v]] = min (adj[type[u]][type[v]], x);
                adj[type[v]][type[u]] = min (adj[type[v]][type[u]], x);
            }
            if(x==0){
                graph[u].pb(v);
                graph[v].pb(u);
        ...
        forn(i,k){
            ll s = pre[i];
            vll visited(n,0);
            dfsvisit(graph, visited, s);
            for(ll j = pre[i]; j < pre[i+1] ; j++){
                if(!visited[j]) flag = 1;
            }
