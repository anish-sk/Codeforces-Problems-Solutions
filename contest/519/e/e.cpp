...
void dfs(vector<vll> &adj,ll u, ll v,vll &parent, vll &depth, vll &count){
    count[u]=1;
    parent[u]=v;
    for(auto i : adj[u]){
        if(i==v) continue;
        depth[i]=depth[u]+1;
        dfs(adj, i, u, parent, depth, count);
        count[u]+=count[i];
    }
} 
...
class LCA {
    public:
        ...
        ll n;
        vector<ll> P, L, C;
        LCA(vector<ll> &parent, vector<ll> &depth, vector<ll> &count){
            n = parent.size();
            P = vector<ll> (all(parent));
            L = vector<ll> (all(depth));
            C = vector<ll> (all(count));
    ...
    ll distance(ll u, ll v){
        ll lc = query(u,v);
        //cout<<"lca = "<<lc<<"\n";
        //cout<<"L[u] = "<<L[u]<<" L[v] = "<<L[v]<<"\n";
        return L[u]+L[v]-2LL*L[lc];
    }
    ll get_node(ll v, ll dis){
        for(int k=19; k>=0; k--){
            if(dis & (1<<k)){
                dis^=(1<<k);
                v = table[v][k];
    ...
    ll co(ll p, ll q){
        ...
        if(d&1) return 0;
        ll a = get_node(p, d/2);
        ll b = get_node(p, d/2 - 1);
        //cout<<"C[a] = "<<C[a]<<" C[b] = "<<C[b]<<"\n";
        ll ans = C[a] - C[b];
        if(L[p] == L[q]){
            ll c = get_node(q, d/2 - 1);
            ans -= C[c];
            ll lc = query(p, q);
            ans += (n - C[lc]);
        }
...
int main()
{
    ...
    while(t--){
        ...
        vll parent(n, 0), depth(n, 0), count(n, 0);
        dfs(adj, 0, -1, parent, depth, count);
        LCA l(parent, depth, count);
        l.build();
        ll m; cin>>m;
        forn(i,m){
            cin>>u>>v;u--;v--;
