...
int main()
{
    ...
    while(t--){
        ...
        vll c1o(n+1,0), c2o(n+1,0), c1e(n+1, 0), c2e(n+1, 0);
        for(int i=n-1; i>=0;i--){
            c1o[i] = c1e[i+1] + (ll)(s[i]=='a'?1:0);
            c1e[i] = c1o[i+1];
            c2e[i] = c2o[i+1];
            c2o[i] = c2e[i+1] +(ll)(s[i]=='b'?1:0);
        }
        //cout<<c1o<<c1e<<c2o<<c2e;
        ll ae = 0, ao= 0;
        for(int i=0;i<n; i++){
            if (s[i]=='a') {ae += c1e[i]; ao +=c1o[i];}
            else {ae+=c2e[i]; ao+= c2o[i];}
        }
        cout<<ae<<" "<<ao<<"\n";
    }
    return 0;
