...
int main()
{
    ...
    //t=1;
    while(t--){
        int n;cin>>n;vi a(n);cin>>a; vi freq(n+1,0); for(auto i:a)freq[i]++;
        sort(all(freq),greater<int>());
        set<int> taken;
        for(auto i: freq){
            while(i>0 && taken.find(i)!=taken.end()) i--;
            if(i>0) taken.insert(i);
        }
        ll sum = 0;
        for(auto i:taken) sum+=i;
        cout<<sum<<"\n";
    }   
    return 0;
}   
