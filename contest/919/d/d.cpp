...
vll topsort;
bool dfsvisit(vector<vll> &adj,vll &visited, int v, vll &col){
    visited[v]=1;
    col[v]=1;
    bool flag = 0;
    for(auto i: adj[v] ){
        if(!visited[i]){
        flag|=dfsvisit(adj, visited, i,col);
        }
        else {
            flag |= (col[i]==1);
        }
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vector<vll> adj(n,vll()); string s;cin>>s;ll u,v;
        forn(i,m){
            cin>>u>>v;
            u--;v--;
            adj[u].pb(v);
        ...
        if(flag){
            ...
            //reverse(all(topsort));
            vector<vll> mx(n,vll(26,0));
            for(auto i: topsort){
                forn(j,26){
                    for(auto v: adj[i]){
                         mx[i][j]=max(mx[i][j],mx[v][j]);
                    }
                    ans=max(ans,mx[i][j]);
                }
                mx[i][s[i]-'a']++;
                ans=max(ans,mx[i][s[i]-'a']);
            }
        }
        cout<<ans<<"\n";
    }
