...
ll nless(ll p, ll m, ll n){
    ll count = 0LL;
    for(ll i= 1LL;i<=m;i++){
        count += min(p/i,n);
        if(p%i == 0 && p/i <= n) count--;
    }
    return count;
}
int main()
{
    ...
    //cout<<l<<" "<<r<<" "<<mid<<"\n";
    mid =(l+r)>>1LL;
    if(nless(mid,m,n)==k-1LL) mid=m*n;
    if(nless(l,m,n)==k-1){
        f=1;
        mid=l;
    }
    else if(nless(r,m,n)==k-1){
        f=1;
        mid=r;
    }
    else if(nless(l,m,n)<k-1 &&nless(r,m,n)>k-1){
        mid = l;
    }
    else if(nless(r,m,n)<k-1 && nless(r+1,m,n)>k-1){
        mid=r;
    }
    cout<<mid<<"\n";
    }
