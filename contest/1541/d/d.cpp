...
void dfs(vector<vll> &adj,ll u, ll v, vll &depth, vll &parent){
    for(auto i : adj[u]){
        if(i==v) continue;
        depth[i]=depth[u]+1;
        parent[i] = u;
        dfs(adj, i, u, depth, parent);
    }
} 
...
class LCA {
    public:
        ...
        ll n;
        vector<ll> P, L;
        LCA(vector<ll> &parent, vector<ll> &depth){
            n = parent.size();
...
int main()
{
    ...
    while(t--){
        ...
        forn(i,n){
            ll curr = 0;
            vll depth(n, 0), parent(n, -1);
            dfs(adj, i, -1, depth, parent);
            LCA l(parent, depth);
            l.build();
            forn(j,n){
                for(int k=j+1; k<n; k++){
                   ll lc = l.query(j,k);
                   ll d1 = depth[j] - depth[lc];
                   ll d2 = depth[k] - depth[lc];
                   curr += f[d1][d2];
                   curr%=p;
                }
            }
