...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n; cin>>n; vll b(n); cin>>b;
        set<ll, greater<ll>> sg;
        set<ll> s;
        s.insert(-1e15); s.insert(1e15); s.insert(b[0]);
        sg.insert(-1e15); sg.insert(1e15); sg.insert(b[0]);
        int i=1;
        bool flag = 1;
        for(; i<n; i++){
            ll up = *s.upper_bound(b[i-1]);
            ll low = *sg.upper_bound(b[i-1]);
            //cout<<low<<" "<<up<<"\n";
            if(b[i] < low){
                flag = 0;
                break;
