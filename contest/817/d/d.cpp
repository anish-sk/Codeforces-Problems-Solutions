...
int main()
{
    ...
    while(t--){
        ...
        ll mans = 0;
        for(int i=1;i<n;i++){
            while(mi.size() && a[i]<a[mi.top()]){
                ll mitop = mi.top();
                mi.pop();
                r[mitop] = i;
                mans += (r[mitop] - l[mitop] - 1LL)*a[mitop];
                mans +=(mitop - l[mitop] -1LL)*(r[mitop] - mitop - 1LL)*a[mitop];
            }
            if(mi.empty()){
                l[i]=-1;
                mi.push(i);
            ...
            while(mx.size() && a[i]>a[mx.top()]){
                ...
                mx.pop();
                mr[mxtop] = i;
                xans += (mr[mxtop] - ml[mxtop] - 1LL)*a[mxtop];
                xans +=(mxtop - ml[mxtop]-1LL)*(mr[mxtop] - mxtop - 1LL)*a[mxtop];
            }
            if(mx.empty()){
                ml[i]=-1;
                mx.push(i);
