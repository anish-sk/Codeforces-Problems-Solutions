...
int main()
{
    ...
    while(t--){
        ...
        ll mb = c[0].f, ma = c[0].s;
        vll bigas, other;
        for(int i=0; i<m;i++){
            if(c[i].s >= mb) bigas.pb(c[i].s);
            else other.pb(c[i].s);
        }
        sort(all(bigas), greater<ll>());
        ll x = bigas.size();
        ll ans = 0;
        if(x >= n){
            ans = accumulate(bigas.begin(), bigas.begin()+n, 0);
        }
        else{
            ans = accumulate(all(bigas),0);
            if(ma >= mb){
                ans = ans + (n-x)*mb;
            }
            else{
