...
void dfs(vector<vi> &adj,int u, int v, vi &depth, vi &count, vi &d, vi &leaf, int &le){
    count[u]=1;
    int j= le;
    for(auto i : adj[u]){
        if(i==v) continue;
        depth[i]=depth[u]+1;
        dfs(adj, i, u, depth, count, d, leaf, le);
        count[u]+=count[i];
        if(d[i]==1) le++;
    }
    if(d[u]==1 && u!=0) leaf[u]=1;
    else leaf[u]=le-j;
}
ll p =1e9+7;
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n;cin>>n; vvi adj(n,vi());int u;vi d(n,0);
        forn(i,n-1){
            cin>>u;u--;
            adj[i+1].pb(u);
            adj[u].pb(i+1);
    ...
    vi depth(n,0), count(n,0), leaf(n,0);int le=0;  
    dfs(adj, 0, -1, depth, count, d, leaf, le);
    map<int, int> subl;
    forn(i,n){
       if(subl.find(count[i]+le-leaf[i])==subl.end()) subl[count[i]+le-leaf[i]]=leaf[i];
       else subl[count[i]+le-leaf[i]]=min(leaf[i],subl[count[i]+le-leaf[i]]);
    }
    sort(all(leaf));
    if(n==1) cout<<"1\n";
    else o1(leaf);
