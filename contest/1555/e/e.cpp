...
int main()
{
    ...
    while(t--){
        ll n,m; cin>>n>>m;
        vector<pair<ll, pll>> inter(n);
        forn(i,n){
            cin>>inter[i].s.f>>inter[i].s.s>>inter[i].f;
            inter[i].s.f*=2;
            inter[i].s.s*=2;
        }
        sort(all(inter));
        int i=0,j=-1;
        seg_tree st((int)2e6+5);
        m*=2;
        st.build(1,1,m);
        st.update(1,1,m,1,1,1);
        ll ans = 1e6;
        while(i<n and j<n){
            while((st.query(1,1,m,1,m) == 0)){
                j++;
                if(j>=n) break;
                st.update(1,1,m,inter[j].s.f, inter[j].s.s, 1);
            }
            if(j < n and i < n)
            ans = min(ans, inter[j].f - inter[i].f);
            st.update(1,1,m,inter[i].s.f, inter[i].s.s, -1);
            i++;
        }
        cout<<ans<<"\n";
    }
