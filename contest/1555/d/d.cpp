...
int main()
{
    ...
    while(t--){
        ...
        forn(i,m){
            ...
            forn(mod, 3){
                forn(j, 3){
                    curr_ct[mod][j] = cnt[mod][j][r] - cnt[mod][j][l-1];
                }
            }
            r--;l--;
            ll reqa,reqb,reqc,extra,an;
            //abc
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            reqc = (r-l+1)/3;
            ...
            an = abs(reqa - curr_ct[l%3][0]) + ((l+1<=r)?(abs(reqb - curr_ct[(l+1)%3][1])):0) + ((l+2<=r)?abs(reqc - curr_ct[(l+2)%3][2]):0);
            ans = min(ans, an);
            //acb
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            ...
            an = abs(reqa - curr_ct[l%3][0]) + ((l+1<=r)?(abs(reqc - curr_ct[(l+1)%3][2])):0) + ((l+2<=r)?abs(reqb - curr_ct[(l+2)%3][1]):0);
            ans = min(ans, an);
            //bac
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            ...
            an = abs(reqb - curr_ct[l%3][1]) + ((l+1<=r)?(abs(reqa - curr_ct[(l+1)%3][0])):0) + ((l+2<=r)?abs(reqc - curr_ct[(l+2)%3][2]):0);
            ans = min(ans, an);
            //bca
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            ...
            an = abs(reqb - curr_ct[l%3][1]) + ((l+1<=r)?(abs(reqc - curr_ct[(l+1)%3][2])):0) + ((l+2<=r)?abs(reqa - curr_ct[(l+2)%3][0]):0);
            ans = min(ans, an);
            //cab
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            ...
            an = abs(reqc - curr_ct[l%3][2]) + ((l+1<=r)?(abs(reqa - curr_ct[(l+1)%3][0])):0) + ((l+2<=r)?abs(reqb - curr_ct[(l+2)%3][1]):0);
            ans = min(ans, an);
            //cba
            reqa = (r-l+1)/3;
            reqb = (r-l+1)/3;
            ...
            //cerr<<reqa<<" "<<reqb<<" "<<reqc<<"\n";
            an = abs(reqc - curr_ct[l%3][2]) + ((l+1<=r)?(abs(reqb - curr_ct[(l+1)%3][1])):0) + ((l+2<=r)?abs(reqa - curr_ct[(l+2)%3][0]):0);
            ans = min(ans, an);
            cout<<ans<<"\n";
        }
    }
