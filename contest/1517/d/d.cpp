...
int main()
{
    ...
    while(t--){
        ll n,m,k; cin>>n>>m>>k;
        vector<vector<pll>> adj(1e6, vector<pll>());
        ll x;
        forn(i,n){
            forn(j, m-1){
                cin>>x;
                adj[i*1000+j].pb({i*1000+j+1, x});
                adj[i*1000+j+1].pb({i*1000+j, x});
            }
        }
        forn(i,n-1){
            forn(j, m){
                cin>>x;
                adj[i*1000+j].pb({(i+1)*1000+j, x});
                adj[(i+1)*1000+j].pb({i*1000+j, x});
            }
        }
        if(k&1){
            forn(i,n){
            ...
            for(int x=2; x<=10; x++){
                forn(i, 1e6){
                    for(auto &[j, y]: adj[i]){
                        dp[i][x] = min(dp[i][x], dp[j][x-1] + y); 
                    }
                }
            }
            forn(i,n){
                forn(j,m){
                    cout<<2LL*dp[i*1000+j][k/2]<<" ";
                }
                cout<<"\n";
            }
