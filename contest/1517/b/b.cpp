...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n, m; cin>>n>>m; vvl b(n, vll(m, 0)); cin>>b;
        vector<pair<ll, pll>> se;
        forn(i,n){
            forn(j, m){
                se.pb({b[i][j], {i, j}});
            }
        ...
        sort(all(se));
        vector<vector<pll>> out(n, vector<pll>(m, {-1,-1}));
        forn(i,m){
            out[se[i].s.f][se[i].s.s] = {i,se[i].f};
        }
        forn(i, n){
            set<ll> st;
            forn(j,m) st.insert(j);
            forn(j,m){
                if(out[i][j].f != -1) st.erase(out[i][j].f);
            }
            forn(j,m){
                if(out[i][j].f == -1){
                    out[i][j] = {*st.begin(), b[i][j]};
                    st.erase(out[i][j].f);
                }
            }
            sort(all(out[i]));
            for(auto j: out[i]) cout<<j.s<<" ";
            cout<<"\n";
        }
    }
