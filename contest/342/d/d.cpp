...
ll p =1e9+7;
ll dp[1<<14][8][2];
ll solve(ll pos, ll mask, ll flag, vector<string> &a, vll &used, ll n){
    ll &ret = dp[pos][mask][flag];
    if(ret == -1LL){
        ret = 0LL;
        if(pos == n){
            ret = (mask == 0LL) and (flag == 1LL);
        }
        else{
            mask |= used[pos];
            if(mask == 7LL){
                ret+= solve(pos+1LL, 0LL, flag, a, used, n);
                return ret;
            }
            if(((mask>>0LL)&1LL) == 0 and ((mask>>1LL)&1LL) == 0){
                ret += solve(pos, mask |(3LL) , flag|(a[2LL][pos] =='O'), a, used, n);
            }
            if(((mask>>1LL)&1LL) == 0 and ((mask>>2LL)&1LL) == 0){
                ret += solve(pos, mask |(6LL) , flag|(a[0LL][pos] =='O'), a, used, n);
            }
            ll i, nflag = 0, nmask=0;
            for(i=0LL; i<3LL; i++){
                if(((mask>>i)&1LL) == 0LL and ((used[pos+1]>>i)&1LL) == 1LL){
                    break;
                }
                if((pos >0 and a[i][pos - 1] == 'O') || (a[i][pos+2LL] == 'O')){
                    nflag = 1;
                }
                if(((mask>>i)&1LL) == 0){
                    nmask |= (1LL<<i);
            ...
            if(i==3LL){
                ret += solve(pos+1LL, nmask, nflag|flag, a, used, n);
            }
        }
        ret %= p;
    }
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n; cin>>n; vector<string> a(4,""); cin>>a[0]>>a[1]>>a[2];
        a[0]+="XX";
        a[1]+="XX";
        a[2]+="XX";
