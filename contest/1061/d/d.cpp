...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,x,y; cin>>n>>x>>y; vector<pair<ll,ll>> a(n); forn(i,n) cin>>a[i].f>>a[i].s;
        vector<pair<ll,ll>> scan; vll st;
        forn(i,n){
            scan.pb({a[i].f, 0});
            scan.pb({a[i].s, 1});
            st.pb(a[i].f);
    ...
    sort(all(st));
    queue<ll> start;
    for(auto i:st) start.push(i);
    ll thres = (x/y);
    ll nt=0; ll cost = 0;ll free=0;ll prev = 0;
    sort(all(scan));
    //cout<<scan;
    sort(all(a));
    multiset<ll> tvs;
    ...
    for(auto &[i,j]: a){
        ll curr_cost = 0;
        if(!tvs.size() || *tvs.begin() >= i){
            curr_cost = x;
        }
        else{
            auto it = tvs.lower_bound(i);
            ll ro = *(--it);
            if((i - ro)>thres){
                curr_cost=x;
            }
