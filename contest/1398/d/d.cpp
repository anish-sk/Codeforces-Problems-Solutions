...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll R,G,B; cin>>R>>G>>B; vll r(R), g(G), b(B); cin>>r>>g>>b;
        int i=0, j=0, k=0;
        sort(all(r));reverse(all(r));
        sort(all(g));reverse(all(g));
        sort(all(b));reverse(all(b));
        ll ans = 0;
        ll F = max(max(R,G),B)+1;
        while(r.size() < F){
            r.pb(0);
        }
        while(g.size() < F){
        ...
        vector<vvl> dp(F+1, vvl(F+1, vll(F+1, 0)));
        for(ll i=F-1;i>=0; i--){
            for(ll j = F-1; j>=0; j--){
                for(ll k = F-1; k>=0; k--){
                    dp[i][j][k] = max(max(dp[i+1][j+1][k] + r[i]*g[j], dp[i][j+1][k+1] + g[j]*b[k]), dp[i+1][j][k+1] + r[i]*b[k]);
