...
inline int left(int i, int n){
    int l = 2*i+1;
    if(l>n) l=n;
    return l;
}
inline int right(int i, int n){
    int r = 2*i+2;
    if(r>n) r=n;
    return r;
}
void calc(int i, vll &a, vector<bool> &ox){
    int s = a.size();
    int l= left(i,s-1);
    int r= right(i,s-1);
    if(!ox[l]){
...
int main()
{
    ...
    while(t--){
        ll n,m; cin>>n>>m;
        n = (1<<n);
        vll a(2*n+1,0); for(int i=n-1;i<2*n-1;i++){cin>>a[i]; }
        vector<bool> ox(2*n+1,0);
        for(int i=n-2;i>=0;i--){
            int l= left(i,2*n-1);
            int r= right(i,2*n-1);
            if(!ox[l]){
