...
int main()
{
    ...
    while(t--){
        int m;cin>>m;
        vector<string> s(m,"");forn(i,m)cin>>s[i];
        map<pair<int,int>,int> count;
        vii nums;
        forn(i,m){
            int a = s[i].find('+');
            int b = s[i].find('/');
            string num1 = s[i].substr(0,a);
            string num2 = s[i].substr(a+1,b-a);
            string den = s[i].substr(b+1);
            num1 = num1.substr(1);
            num2.pop_back();num2.pop_back();
            //cout<<num1<<" "<<num2<<" "<<den<<"\n";
            stringstream n1(num1),n2(num2),d(den);
            float nu1,nu2,d2;
            n1>>nu1;n2>>nu2;d>>d2;
            float n = nu1+nu2;n*=100;
            int nm = (int)n;
            int dm = (int)d2*100;
            int g =__gcd(nm,dm);
            if(g!=0){
