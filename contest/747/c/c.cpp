...
ll p =1e9+7;
int server_free(vi &server, int t, int k, vi & tupush){
    int i=0;
    int count=k;
    vi curr;
    for(;i<server.size();i++){
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,q; cin>>n>>q; vector< pair<int, pair<int,int>>> task(q);
        forn(i,q) cin>>task[i].f>>task[i].s.f>>task[i].s.s;
        sort(all(task));
        vi server(n,0);
        vi ans;
        for (auto &[t,kd] : task){
            ...
            int k = kd.f;
            int d = kd.s;
            int j = server_free(server,t,k, tupush);
            if(j!=-1){
                int count= 0;
                for(auto i: tupush){
                    server[i]=t+d-1;
