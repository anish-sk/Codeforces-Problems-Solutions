...
void dfsvisit(vvl &adj,vll &visited, ll v, bool &flag, vll &topo){
    visited[v]=1;
    for(auto i: adj[v]){
        if(visited[i]==0){
        dfsvisit(adj, visited, i, flag, topo);
        }
        else if(visited[i]==1){
            flag = 1;
        }
...
int main()
{
    ...
    while(t--){
        ...
        forn(i,n) cin>>a[i];
        vll parent(n+m), size(n+m);
        forn(i, n+m) make_set(i, parent, size);
        forn(i,n){
            forn(j,m){
                if(a[i][j] == '='){
                    union_sets(i, j+n, parent,  size);
                } 
            }
        }
        bool flag  = 0;
        unordered_set<ll> comps;
        forn(i,n+m) comps.insert(find_set(i, parent));
        ll nc = comps.size();
        vvl adj(n+m, vll());
        forn(i,n){
            forn(j,m){
                if(a[i][j]!= '='){
                    ll c = find_set(i, parent);
                    ll d = find_set(j+n, parent);
                    if(c==d){
                        flag = 1;
                    }
                    if(a[i][j] == '>'){
        ...
        if(flag){
            ...
            forn(i,n){
                cout<<col[find_set(i, parent)]<<" ";
            }
            cout<<"\n";
            forn(j,m){
                cout<<col[find_set(j+n, parent)]<<" ";
            }
            cout<<"\n";
        }
    }
