...
map<pll, ll> vmap;
map<pll, ll> edge_no;
map<pll, vll> edge_nos;
vector<pll> matchings;
void dfsvisit(vector<vector<pll>> &adj,vll &visited, ll u, ll v, vll &level, vll &rem){
    visited[u]=1;
    ll curr_unmatch = -1;
    for(auto &[i,j]: adj[u]){
        ...
        if(!visited[i]){
            level[i] = 1 + level[u];
            dfsvisit(adj, visited, i, u, level, rem);
            if(rem[i] != -1){
                matchings.pb({nw , rem[i]});
                nw = -1;
            }
        }
        if(nw != -1){
            if(curr_unmatch != -1){
                matchings.pb({nw , curr_unmatch});
                curr_unmatch = -1;
            }
            else{
                curr_unmatch = nw;
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n; cin>>n; vector<pair<pll,pll>> a(n);
        ll curr = 0;
        vector<pair<pll, ll>> edge_list;
        forn(i,n){
            cin>>a[i].f.f>>a[i].f.s>>a[i].s.f>>a[i].s.s;
            ll le = a[i].f.s * a[i].s.f;
            ll ri = a[i].s.s * (a[i].f.f + a[i].f.s);
            ll g = __gcd(le, ri);
            le/=g;ri/=g;
            if(!vmap.count({le, ri})){
                vmap[{le, ri}] = curr;
            ...
            ll se = vmap[{le, ri}];
            edge_list.pb({{fi, se}, i+1});
            edge_no[{fi, se}]=i+1;
            edge_no[{se, fi}]=i+1;
            //edge_nos[{fi, se}].pb(i+1);
            //edge_nos[{se, fi}].pb(i+1);
           
        }
        ll nodes = vmap.size();
        vector<vector<pll>> adj(nodes, vector<pll>());
        vector<vector<pll>> adj2(nodes, vector<pll>());
        for(auto pa: edge_list){
            adj[pa.f.f].pb({pa.f.s, pa.s});
            adj[pa.f.s].pb({pa.f.f, pa.s});
        }
        vll visited(nodes, 0), level(nodes, 0), rem(nodes, -1);
        forn(i, nodes){
            if(!visited[i]){
                dfsvisit(adj, visited, i, -1, level, rem);
            }
        }
        cout<<matchings.size()<<"\n";
        for(auto pa: matchings){
            cout<<pa.f<<" "<<pa.s<<"\n";
        }
