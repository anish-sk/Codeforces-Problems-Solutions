...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n; cin>>n; vll u(n), s(n); cin>>u>>s;
        ll sum = 0;
        for(auto i: s) sum += i;
        cout<<sum<<" ";
        set<ll> rem;
        vvl players(n, vll());
        forn(i,n){
            players.at(u[i]-1).pb(s[i]);
        }
        forn(i,n) sort(all(players[i]), greater<ll>());
        forn(i,n){
            ll j = (ll)players[i].size()-2;
            while(j>=0){
                players[i][j]+=players[i][(j+1)];
                j--;
            }
        }
        forn(i,n) if(players[i].size())rem.insert(i);
        ll curr_sum = sum;
        //cout<<players;
        for(int k=2; k<=n; k++){
            curr_sum = sum;
            vll to_be;
            for(auto j: rem){
                ll po = players[j].size()%k;
                if(po > 0){
                    po = players[j].size() - po;
                    curr_sum-=players[j].at(po);
                }
                if(k >= (int)players[j].size()) to_be.pb(j);
            }
            cout<<curr_sum<<" ";
            for(auto j: to_be) {
                rem.erase(j);
