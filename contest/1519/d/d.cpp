...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n; cin>>n; vll a(n), b(n); cin>>a>>b;
        vll c(n+2, 0LL);
        forn(i,n) c[i+1]=c[i] + a[i]*b[i];
        vvl sum(n, vll(n, 0));
        forn(j,n){
            vll d = b;
            reverse(d.begin(), d.begin()+j+1);
            ll s = 0;
            forn(i,n) s+=a[i]*d[i];
            sum[0][j] = s;
            sum[j][j] = a[j]*b[j];
        ...
        forn(j,n){
            vll d = b;
            reverse(d.begin()+j, d.end());
            ll s = 0;
            forn(i,n) s+=a[i]*d[i];
            sum[j][n-1] = s; 
        }
        //cout<<sum;
        ll mx = diags(0LL,n-1,c);
        for(int i=n-2; i>=0; i--){
            for(int j=i+1; j<n; j++){
                sum[i][j] = sum[i+1][j-1] + a[i]*b[j] + a[j]*b[i]; 
                mx = max(mx, diags(0LL,n-1, c) - diags(i,j, c) + sum[i][j]);
            }
        }
