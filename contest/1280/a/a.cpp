...
int main()
{
    ...
    while(t--){
        ...
        ll ans = 0;
        ans=s.size();
        while(cursor < x and s.size() <=x){
            //cout<<s<<" "<<ans<<endl;
            //s=s+string(s.substr(cursor), s[cursor]-1);
            string y = s.substr(cursor+1);
            forn(_, s[cursor]-1-'0') s+=y;
            ans=s.size();
            cursor++;
        }
        while(cursor < x){
            //cout<<ans<<endl;
            ans += ((ans - cursor - 1+p)%p)*(s[cursor]-1-'0');
            ans%=p;
            cursor++;
        }
        cout<<ans<<"\n";        
