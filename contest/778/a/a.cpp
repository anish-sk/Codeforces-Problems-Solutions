...
int main()
{
    ...
    //cin>>t;
    while(t--){
        string s,p; cin>>s>>p;ll n = s.size(); vll a(n); cin>>a;
        ll l=0, r=s.size(); ll mid = (l+r)/2;
        while(l<r){
            mid = (l+r)/2;
            string u=""; vll b(n,1);
            for(int i=0;i<mid;i++){
            ...
            //cout<<mid<<" "<<u<<"\n";
            if(isSubSequence(p, u, p.size(), u.size())) l= mid+1;
            else r = mid-1;
        }
        //cout<<mid<<"\n";
        ll lower = max(mid -10, 0LL), upper = max(mid + 10, n);
        ll ans = n;
        for(ll mid=lower; mid<=upper; mid++){
            string u=""; vll b(n,1);
            for(int i=0;i<mid;i++){
                b[a[i]-1]=0;
            }
            ...
            if(!isSubSequence(p,u,p.size(),u.size())){ans = mid-1; break;}
        }
        cout<<ans<<"\n";
    }
    return 0;
