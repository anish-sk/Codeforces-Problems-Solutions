...
ll p =1e9+7;
bool inters(vector<pair<pair<ll,ll>, ll > > &c, vector<pair<pair<ll,ll> , ll> > &b){
    multiset<ll> end;
    multiset<ll> start;
    bool flag = 1;
    sort(all(c));
    for(auto &[x,y] : c){
        if(x.s == 1){
            if(start.size()){
                ll smax = *(start.rbegin());
                ll emin = *(end.begin());
                if(smax > b[y].f.s or emin < b[y].f.f){
                    return false;
                }
            }
            start.insert(b[y].f.f);
            end.insert(b[y].f.s);
        }
        else{
            start.erase(start.find(b[y].f.f));
            end.erase(end.find(b[y].f.s));
        }
    }
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n; cin>>n;  vector<pair<pair<ll,ll>, ll > > a(n), b(n), c,d; forn(i,n) {cin>>a[i].f.f>>a[i].f.s>>b[i].f.f>>b[i].f.s; a[i].s=i; b[i].s = i;}
        for(auto &[x,y] : a){
            c.pb({{x.f, 1},y});c.pb({{x.s+1, 0},y});
        }
        for(auto &[x,y] : b){
            d.pb({{x.f, 1},y});d.pb({{x.s+1, 0},y});
        }
        if(inters(c,b) and inters(d,a)){
            cout<<"YES\n";
        }
        else cout<<"NO\n";
    }
