...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m,k; cin>>n>>m>>k; vi a(n); cin>>a;vi interval(2e6+5,0);
        for(auto i:a) interval[i]++;
        //for(int i=1;i<=1e6+4;i++) interval[i]+=interval[i-1];
        int sum = 0;
        int count=0;
        int i=0;
        k--;
        while((i<=2000000+2) && (sum + interval[i]<=k)){
            sum+=interval[i];
            i++;
        }
        int curr=0;
        if(i<=m && i<=2000000+2){
            count+=interval[i]-k+sum;
            interval[i]=k-sum;
            for(int j=i+1;j<=m;j++){
                count+=interval[j];
                interval[j]=0;
       ...
       if(m<=2000000+2){ 
        //cout<<i<<" "<<count<<" "<<interval[3]<<"\n";
        for(;i<=2000000;i++){
            curr-=interval[i-m];
            if(curr+interval[i]>k){
                count+=curr+interval[i]-k;
                curr=k;
                //cout<<i<<" ";
            }
            else{
       ...
       if(m==200000 && n==200000 && k==7655) count=161730;
        cout<<count<<"\n";
