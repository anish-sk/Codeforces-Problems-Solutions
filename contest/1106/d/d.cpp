...
void dfsvisit(vvi &adj,vi &visited, int v, vi &col, vi &order){
    order.pb(v);
    visited[v]=1;
    for(auto i: adj[v]){
        if(!visited[i]){
        col[i] = 1 - col[v];
        dfsvisit(adj, visited, i, col,order);
        }
    }
}
...
int main()
{
    ...
    t=1;
    while(t--){
        int m,n;cin>>n>>m;vvi adj(n,vi());int u,v;
        forn(i,m){
            cin>>u>>v;u--;v--;
            adj[u].pb(v);
            adj[v].pb(u);
        }
        vi order,visited(n,0); priority_queue<int,vector<int>,greater<int>> q;q.push(0);visited[0]=0;
        ...
        while(q.size()){
            ...
            visited[u]=1;
            for(auto v: adj[u]){
                if(!visited[v]&&(pushed.find(v)==pushed.end())){
                q.push(v);
                pushed.insert(v);
                }
             }
