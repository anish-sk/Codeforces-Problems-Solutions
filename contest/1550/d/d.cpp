...
ll fact[(ll)2e5 + 5], inv[(ll)2e5+5];
ll ncr(ll n, ll r){
    if(n<r || r < 0LL) return 0LL;
    return (((fact[n] * inv[n-r])%p)*(inv[r])%p)%p;
}

ll ways(ll fp, ll fn, ll ei){
    ll n = fp + fn + ei;
    if(n%2 == 0){
        return ncr(n-fp-fn, n/2-fp);
    }
    ll k1 = n/2 + 1;
    ll k2 = n/2;
    ll ans = ncr(n-fp-fn, k1 - fp);
    ans += ncr(n-fp-fn, k2 - fp);
    ans %= p;
    return ans;
}
...
int main()
{
    ...
    while(t--){
        ll n,l,r; cin>>n>>l>>r;
        ll ans = 0;
        ll nskip = min(1LL-l, r-n);
        if(nskip>0){
            ll x = ways(0LL,0LL,n) * nskip;
            x%=p;
            ans += x;
            ans %= p;
        }
        for(ll offset = nskip + 1LL; ; offset++){
            ll fp = max(l+offset-1LL, 0LL);
            ll fn = max(offset-(r-n), 0LL);
            ll ei = n - fp - fn;
            if(ei < 0) break;
            ans+=ways(fp, fn, ei);
            ans %= p;
