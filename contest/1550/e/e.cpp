...
bool solve(string &s, ll jump, ll k, ll n){
   vvl prefix(k, vll(n+1, 0)); 
   for(int i=1; i<=n;  i++){
       for(int c = 0; c<k; c++){
           prefix[c][i] = prefix[c][i-1];
           if(s[i-1] == (c+'a') or s[i-1] == '?'){
               prefix[c][i]++;
           }
       }
   }
   vvl next_place(k, vll(n+1, 0)); 
   for(int c=0; c<k; c++) next_place[c][n] = n+1;
   for(int i=n-1; i>=0; i--){
       for(int c=0; c<k; c++){
           next_place[c][i] = next_place[c][i+1];
           if(i+jump <= n and prefix[c][i+jump] - prefix[c][i] == jump){
               next_place[c][i] = i;
           }
       }
   }
   ...
   for(int mask = 1; mask < (1<<k); mask++){
       ...
       for(int last = 0; last<k; last++){
            ...
            ll from_end = early[from];
            if(from_end >= n) continue;
            ll start = next_place[last][from_end];
            early[mask] = min(early[mask], start + jump);
       }
   }
   return early[(1<<k) - 1]<=n;
}
int main()
{
