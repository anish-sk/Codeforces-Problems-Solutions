...
{ 
    ...
    // number as 2 
    for (int i=4LL; i<MAXN; i+=2LL) 
        spf[i] = 2LL; 
  
    for (int i=3LL; i*i<MAXN; i++) 
    { 
...
void getFactorization(ll x, set<ll> &ret) 
{ 
    while (x != 1LL) 
    { 
        ret.insert(spf[x]); 
