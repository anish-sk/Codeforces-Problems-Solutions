...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vll p(n), c(n); vll votes(m,0); forn(i,n) {cin>>p[i]>>c[i];p[i]=p[i]-1;votes[p[i]]++;}
        vector<vector<pair<ll,ll> > > bank(m, vector<pair<ll,ll>>());
        vll vt = votes;
        forn(i,n){
            bank[p[i]].pb({c[i],i});
        }
        ...
        for(ll k=1; k<= n/2 + 1; k++){
            ...
            if(need_votes> 0){
                vll rem;
                forn(i,n){
                    if(mark[i] == 0 and p[i] > 0) rem.pb(c[i]);
                }
                sort(all(rem));
                forn(i, need_votes) d+=rem[i];
            }
            cost = min(cost, d);
        }
        cout<<cost<<"\n";
