...
bool inter(pll a, pll b){
    ...
    if(a1 > b1) swap(a1, b1);
    if(a2 > b2) swap(a2, b2);
    if(a2 > a1 and a2 < b1 and b1 < b2) return true;
    if(a1 > a2 and a1 < b2 and b2 < b1) return true;
    return false;
}
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n,k; cin>>n>>k; vector<pll> points(k);
        set<ll> st;
        forn(i,2LL*n) st.insert(i+1);
        forn(i,k){
        ...
        forn(i,n){
            for(int j=i+1; j<n; j++){
                bool c =  inter(points[i], points[j]);
                //if(c) cout<<points[i]<<points[j]<<"\n";
                ans += c;
            }
        }
        cout<<ans<<"\n";
