...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,k; cin>>n>>k; vll c(n*k); cin>>c;
        set<ll> rem;
        ...
        forn(y, k-1){
            ll to_take = ceil(n,(k-1));
            vector<pll> curr;
            for(auto col: rem){
                curr.pb({a[col][y+1], col});
            }
            sort(all(curr));
            forn(i, min(to_take, (ll)curr.size())){
                ans[curr[i].s] = {a[curr[i].s][y]+1, a[curr[i].s][y+1] + 1};
                rem.erase(curr[i].s);
            }
        }
        for(auto pa: ans) cout<<pa.f<<" "<<pa.s<<"\n";
    }
