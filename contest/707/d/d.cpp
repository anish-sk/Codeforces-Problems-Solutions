...
int main()
{
    ...
    //cin>>t;
    while(t--){
        int n,m,q; cin>>n>>m>>q; vector<vector<int>> pos(q+1, vector<int>(n+1,0));
        vi k; k.pb(0);
        vector<string> lines;
        string c = string(m, '0');
        lines.pb(c);
        for(int l=1;l<=q; l++){
            ll op; cin>>op;
            if(op == 1 ){
                for(int i=1; i<=n; i++) pos[l][i] = pos[l-1][i];
                ll i,j; cin>>i>>j;j--;
                string b = lines[pos[l][i]];
                int ones = 0;
