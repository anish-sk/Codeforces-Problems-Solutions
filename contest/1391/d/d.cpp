...
ll cnt(vvl &a, ll n, ll m){
    vll mask(m,0);
    for(ll i=0; i<m; i++){
        ll maask = 0;
        for(ll j=0;j<n; j++){
            if(a[j][i] == 1) maask += (1LL<<j);
        }
        mask[i] = maask;
    }
    vvl dp(m, vll(1LL<<n,1e9));
    for(int i=0; i<(1LL<<n); i++){
        dp[0][i] = bitc(i^mask[0]);
    }
    dp[0][mask[0]] = 0;
    vvl possible_transition(1LL<<n, vll(1LL<<n, 0));
    for(ll m1=0; m1<(1LL<<n); m1++){
        for(ll m2 = 0; m2<(1LL<<n); m2++){
            vvl board(n, vll(2,0));
            for(ll i=0;i<n; i++){
                    board[i][0]=(m1&(1LL<<i))!=0;
                    board[i][1]=(m2&(1LL<<i))!=0;
            }
            possible_transition[m1][m2] = works(board);                
        }
    }
    for(ll i=1; i<m; i++){
        for(ll to = 0; to < (1<<n); to++){
            for(ll from = 0; from < (1<<n); from++){
                if(possible_transition[from][to]){
                    dp[i][to] = min(dp[i][to], dp[i-1][from] + bitc(to^mask[i]));
                }
            }
        }
    }
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vvl a(n, vll(m,0)); 
        forn(i,n){
            string s; cin>>s;
            forn(j,m){
                a[i][j] = s[j]-'0';
