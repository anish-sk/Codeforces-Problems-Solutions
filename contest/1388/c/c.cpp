...
void dfs(vector<vll> &adj,ll u, ll v, vll &g, vll &b, vll &p, vll &h, bool &flag){
    ll gsum = 0;
    ll bsum = 0;
    for(auto i : adj[u]){
        if(i==v) continue;
        dfs(adj, i, u, g, b, p, h, flag);
        p[u] += p[i];
        gsum += g[i];
        bsum += b[i];
    }
    g[u] = (p[u] + h[u])/2LL;
    b[u] = (p[u] - h[u])/2LL;
    if(g[u] + b[u] != p[u]) flag = 1;
    ll x = p[u] + h[u];
    if(x&1LL) flag = 1;
    if(g[u]<0 or b[u]<0) flag = 1;
    if(gsum > g[u]) flag = 1;
    //if(bsum < b[u]) {cout<<u<<" "<<b[u]<<" "<<bsum<<endl;flag = 1;}
} 
...
int main()
{
    ...
    cin>>t;
    while(t--){
        ll n,m; cin>>n>>m; vvl adj(n, vll()); 
        vll p(n); cin>>p; vll h(n); cin>>h;
        forn(i,n-1){
            ll u,v; cin>>u>>v; u--;v--;
            adj[u].pb(v);adj[v].pb(u);
        }
        vll count(n,0), g(n,0), b(n,0);bool flag = 0;
        dfs(adj, 0, -1,  g, b, p, h, flag);
        //cout<<g<<b;
        cout<<(flag?"NO":"YES")<<"\n";
    }
    return 0;
