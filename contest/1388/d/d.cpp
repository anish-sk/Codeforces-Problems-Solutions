...
void dfsvisit(vvl &adj,vll &visited, ll v, vll &order ){
    visited[v]=1;
    for(auto i: adj[v]){
        if(!visited[i]){
        dfsvisit(adj, visited, i, order );
        }
    }
    order.pb(v);
}
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n; cin>>n; vll a(n); cin>>a; vll b(n); cin>>b;
        vvl adj(n, vll());
        forn(i,n){
            if(b[i] == -1) continue;
            b[i] --;
        ...
        ll ans = 0;
        vll visited(n,0); vll order; vll op;
        forn(i,n){
            if(!visited[i])
            dfsvisit(adj, visited, i, order);
        }
