...
ll p =1e9+7;
void dfs(vector<vll> &adj,ll u, ll v, vll &count){
    count[u]=1;
    for(auto i : adj[u]){
        if(i==v) continue;
        dfs(adj, i, u, count);
...
int main()
{
    ...
    while(t--){
        ...
        ll ans = 0;
        forn(i,n){
            vvl ad(n, vll()); vll count(n,0);vll visited(n,0);
            queue<ll> q; q.push(i);ll val = a[i];
            while(q.size()){
                ll u = q.front();q.pop();
                visited[u]=1;
                for(auto v: adj[u]){
                    if(visited[v]) continue;
                    if(a[v] > val and a[v] <= val+d){
                        ad[u].pb(v);
                        q.push(v);
                    }
                    else if(a[v] == val and v > i){
                        ad[u].pb(v);
                        q.push(v);
                    }
                }
