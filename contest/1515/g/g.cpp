...
void dfs_fwd(vector<vector<pll>> &adj,vll &visited, ll v, vll &ord){
    visited[v]=1;
    for(auto &[i,j]: adj[v]){
        if(!visited[i]){
            dfs_fwd(adj, visited, i, ord);
        }
    }
    ord.pb(v);
}
void dfs_rev(vector<vector<pll>> &adj,vll &visited, ll v, vll &offset, vll &loop, vll &cc){
    visited[v]=2;
    for(auto &[i,j]: adj[v]){
        if(visited[i]!=2){
            cc[i] = cc[v];
            offset[i] = offset[v] + j;
            dfs_rev(adj, visited, i, offset, loop, cc);
        }
        else if(cc[i] == cc[v]){
            loop[cc[v]] = gcd(loop[cc[v]], llabs(offset[v] + j - offset[i]));
        }
    }
}
class seg_tree{
...
int main()
{
    ...
    while(t--){
        ...
        vector<vector<pll>> adj_rev(n, vector<pll>());
        forn(i,m){
            ll a,b,l; cin>>a>>b>>l;
            a--;b--;
            adj[a].pb({b, l});
        ...
        forn(i,q){
            ll a,b,l;
            cin>>a>>b>>l;
            v.pb(a-1);s.pb(b);rt.pb(l);
        }
        vll visited(n, 0), offset(n, 0), loop(n, 0), cc(n, 0); 
        vll ord;
        forn(i,n){
            if(visited[i]!=1){
                dfs_fwd(adj, visited, i, ord);
        ...
        for(auto i: ord){
            if(visited[i] != 2){
                cc[i] = ncc++;
                offset[i] = 0;
                dfs_rev(adj_rev, visited, i, offset, loop, cc);
            }
        }
        forn(i,q){
            if(s[i]%(gcd(loop[cc[v[i]]], rt[i])) == 0){
                cout<<"YES\n";
            }
            else{
                cout<<"NO\n";
