...
ll front, back;
void dfsvisit(vvl &adj,vll &visited, ll v, vll &ans, map<pll,ll> &etoi, vll &a,ll x){
    visited[v]=1;
    for(auto i: adj[v]){
        if(!visited[i]){
            dfsvisit(adj, visited, i, ans, etoi, a, x);
            if(a[v] + a[i] >= x){
                a[v]+=(a[i]-x);
                ans[front++]=etoi[{v, i}];
            }
...
int main()
{
    ...
    //cin>>t;
    while(t--){
        ll n, m, x; cin>>n>>m>>x; vll a(n); cin>>a;
        ll sum = 0;
        forn(i,n) sum+=a[i];
        if(sum < (n-1LL)*x){
            cout<<"NO\n";
        ...
        vll visited(n, 0), ans(n, 0);
        front  = 0;
        back = n-1;
        dfsvisit(adj, visited, 0, ans, etoi, a, x);
