...
int main()
{
    ...
    while(t--){
        ll n, M; cin>>n>>M;
        p = M;
        vll two(n+1, 0), fact(n+1, 0), inv(n+1, 0);
        fact[0] = 1;
        two[0] = 1;
        inv[0] = 1;
        forn(i,n){
            ...
            fact[i+1]  = fact[i] * (i+1);
            fact[i+1] %= p;
            inv[i+1] = MINV(fact[i+1], p);
        }
        vvl choose(n+1, vll(n+1, 0));
        forn(i, n+1){
            for(int j=0; j<=i; j++){
                choose[i][j] = (fact[i]*inv[j])%p;
                choose[i][j] *= inv[i-j];
                choose[i][j] %= p;
            }
        }
        vvl dp(n+2, vll(n+1, 0));
        dp[0][0] = 1;
        for(int i=0; i < n; i++){
            for(int j=0; j< i+1; j++){
                for(int k=1; i+k<n+1; k++){
                    ll term = choose[j+k][k] * dp[i][j];
                    term %= p;
                    term *= two[k-1];
                    term %= p;
                    dp[i+k+1][j+k] += term;
