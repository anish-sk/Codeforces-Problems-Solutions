...
class seg_tree{
    ...
    void add(int si, int ss, int se, int pos, ll new_val){
        ...
        int mid = (ss + se)/2;
        if(pos <= mid){
            add(2*si, ss, mid, pos, new_val);
        }
        else{
            add(2*si+1, mid+1, se, pos, new_val);
        }
        st[si] = st[2*si] + st[2*si+1];
    }

    void delet(int si, int ss, int se, int k){
        //cout<<"ss = "<<ss<<" se = "<<se<<" si = "<<si<<"\n";
        if(ss == se){
            st[si] = 0;
            val[si] = 0;
        ...
        if(st[2*si] >= k){
            ...
            delet(2*si+1, mid+1, se, k-st[2*si]);
        }
        st[si] = st[2*si] + st[2*si+1];
    }
    int get_max(int si, int ss, int se){
        if(ss > se) return 0;
        if(ss == se) return ss;
        int mid = (ss + se)/2;
        if(st[2*si+1]) return get_max(2*si+1, mid+1, se);
        else return get_max(2*si, ss, mid);
    }
    void print(int si, int ss, int se){
        if(ss > se || !st[si]) return;
        if(ss == se){
            if(st[si]!=0) cout<<val[si];
            return;
        }
        int mid = (ss + se)/2;
        print(2*si, ss, mid);
    ...
    while(t--){
        ...
        forn(i,n){
            if(events[i] == -1){
                ...
                ll pos;
                if(curr == 1) pos = 1;
                else pos = s.get_max(1,1,n)+1;
                s.add(1,1,n,pos, events[i]);
            }
            //cout<<curr<<"\n";
            //cout<<s.st;
        }
